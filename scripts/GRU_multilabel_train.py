...
OUTPUT_BASE_DIR = f"/kaggle/working/gru_multi_results_ws{RUN_WINDOW_SIZE}"
...
def build_gru(input_shape, n_units=64, n_layers=1, dropout=0.3,
              lr=1e-3, l2_reg=1e-4, use_bidirectional=False):
    ...
    for i in range(n_layers):
        ...
        if use_bidirectional:
            x = layers.Bidirectional(gru_layer, name=f"bigru_{i}")(x)
        else:
            x = gru_layer(x)
    ...
    x   = layers.Dense(max(16, n_units // 2), activation="relu",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name="dense_head")(x)
    ...
    model = models.Model(inp, out, name=f"gru_multi_ws{RUN_WINDOW_SIZE}")
    model.compile(
...
def make_objective(X_tr, y_tr_ch, y_tr_bin, X_val, y_val_ch, y_val_bin):
    def objective(trial):
        ...
        use_bidir  = trial.suggest_categorical("bidirectional", [True, False])

        model = build_gru(
            (X_tr.shape[1], X_tr.shape[2]),
            n_units, n_layers, dropout, lr, l2_reg, use_bidir,
        )
...
def run_optuna(X_tr, y_tr_ch, y_tr_bin, X_val, y_val_ch, y_val_bin):
    db   = os.path.join(OUTPUT_BASE_DIR, f"optuna_gru_ws{RUN_WINDOW_SIZE}.db")
    name = f"gru_multi_ws{RUN_WINDOW_SIZE}"
    ...
    if rem > 0:
        logger.info(f"Optuna GRU: {done} prev, {rem} rest...")
        study.optimize(
...
def save_results(model, X_te, yc_te, yb_te, best_threshold,
    ...
    metrics = {
        "model": "GRU", "window_size": RUN_WINDOW_SIZE,
    ...
    logger.info(f"\nGRU — ws={RUN_WINDOW_SIZE}  bidireccional={best_params.get('bidirectional')}")
    ...
    with open(os.path.join(out_dir, "classification_report.txt"), "w") as f:
        f.write(f"Model: GRU (bidireccional={best_params.get('bidirectional')})\n")
        f.write(f"Window: {RUN_WINDOW_SIZE}\n")
        f.write(f"Macro-F1: {macro_f1:.4f}\nMicro-F1: {micro_f1:.4f}\n")
        f.write(f"Hamming: {h_loss:.4f}\nSize KB: {keras_size_kb:.4f}\n\n")
        f.write(report)

    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle(f"GRU Multilabel — ws={RUN_WINDOW_SIZE}  Macro-F1={macro_f1:.4f}", fontsize=13)
    if "loss" in histd:
    ...
    axes[2].set_title("F1 por canal"); axes[2].legend()
...
def main():
    ws = RUN_WINDOW_SIZE
    logger.info(f"GRU MULTILABEL (GPU) — window_size={ws}")
    ...
    if os.path.exists(flag) and os.path.exists(ckpt):
        ...
        model = build_gru(
            ...
            use_bidirectional = best_params.get("bidirectional", False),
        )
