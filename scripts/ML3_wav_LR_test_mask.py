...
MODEL_DIR = "./models"
RESULTS_DIR = "./results/ML3_wav_LR_mask"
...
# Mask to test only in anomalies
mask_anom = y_test == 1
E_test_anom = E_test[mask_anom]
y_ch_test_mask = y_ch_test[mask_anom]

y_pred = np.zeros_like(y_ch_test_mask)
for ch in range(15):
    prob = models[ch].predict_proba(E_test_anom)[:,1]
...
report = classification_report(
    y_ch_test_mask,
    y_pred,
    target_names=[f"ch{i}" for i in range(y_ch_test_mask.shape[1])],
    digits=4,
    output_dict=True
)
print(report)
...
conf_mats = {}
for ch in range(y_ch_test_mask.shape[1]):
    cm = confusion_matrix(y_ch_test_mask[:, ch], y_pred[:, ch])
